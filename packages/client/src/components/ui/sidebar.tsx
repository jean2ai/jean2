  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      <div
        ...
        className={cn( 
          "overflow-hidden group/sidebar-wrapper flex h-full w-full has-data-[variant=inset]:bg-sidebar",
          className
        )}
        {...props}
      >
  ...
  ({ className, ...props }, ref) => {
    return (
      <div
        ...
        className={cn(
          "no-scrollbar flex min-h-0 min-w-0 w-full flex-1 flex-col gap-0 overflow-y-auto overflow-x-hidden group-data-[collapsible=icon]:overflow-hidden",
          className
        )}
        ref={ref}
        {...props}
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn( 
        "flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-200 ease-linear group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
        className
      )}
      {...props}
    />
