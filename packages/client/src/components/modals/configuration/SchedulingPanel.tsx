...
const RISK_OPTIONS: { value: PermissionRiskLevel; label: string; description: string }[] = [
  { value: 'none', label: 'None', description: 'Auto-approve all operations' },
  { value: 'low', label: 'Low', description: 'Only ask for destructive operations' },
  { value: 'medium', label: 'Medium', description: 'Ask for each operation' },
...
export function SchedulingPanel({ enabled, permissionRisk, onChange }: SchedulingPanelProps) {
  return (
    <div className="p-3 sm:p-4 space-y-6">
      <div className="flex items-center justify-between">
        <div className="space-y-0.5">
          <Label htmlFor="scheduling-enabled">Enable scheduler</Label>
          <p className="text-xs text-muted-foreground">
            Allow the agent to create, manage, and trigger scheduled tasks
          </p>
        </div>
        <Switch
      ...
      {enabled && (
        <div className="space-y-3">
          <Label>Permission level for operations</Label>
          <p className="text-xs text-muted-foreground">
            Controls when the agent needs approval to manage schedules
          </p>
