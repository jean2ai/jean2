...
import {
  ...
  getWorkspacePermissionsHistory,
  revokeAllWorkspacePermissions,
...
describe('permissions store', () => {
  ...
  describe('legacy permissions', () => {
    test('grantPermission and checkCachedPermission roundtrip', () => {
      seedWorkspace({ id: 'ws1' });
      grantPermission({
        workspaceId: 'ws1',
        toolName: 'shell',
        permissionType: 'shell-command',
        permissionKey: 'npm test',
        allowed: true,
        grantedBy: null,
      });

      const result = checkCachedPermission('ws1', 'shell', 'shell-command', 'npm test');
      expect(result).not.toBeNull();
      expect(result!.allowed).toBe(true);
    });

    test('checkCachedPermission returns null when no permission', () => {
      seedWorkspace({ id: 'ws1' });
      expect(checkCachedPermission('ws1', 'shell', 'shell-command', 'npm test')).toBeNull();
    });

    test('revokePermission marks permission as revoked', () => {
      seedWorkspace({ id: 'ws1' });
      ...
      revokePermission(perm.id, null);
      expect(checkCachedPermission('ws1', 'shell', 'shell-command', 'npm test')).toBeNull();
    });

    test('getWorkspacePermissions lists active permissions', () => {
      seedWorkspace({ id: 'ws1' });
      grantPermission({
        workspaceId: 'ws1',
        toolName: 'shell',
      ...
      expect(getWorkspacePermissions('ws1')).toHaveLength(2);
    });

    test('getWorkspacePermissions excludes revoked by default', () => {
      seedWorkspace({ id: 'ws1' });
      const perm = grantPermission({
        workspaceId: 'ws1',
        toolName: 'shell',
      ...
      revokePermission(perm.id, null);
      expect(getWorkspacePermissions('ws1')).toHaveLength(0);
    });

    test('getWorkspacePermissions includes revoked when flag set', () => {
      seedWorkspace({ id: 'ws1' });
      const perm = grantPermission({
        workspaceId: 'ws1',
        toolName: 'shell',
      ...
      revokePermission(perm.id, null);
      expect(getWorkspacePermissions('ws1', true)).toHaveLength(1);
    });

    test('getWorkspacePermissionsHistory limits results', () => {
      seedWorkspace({ id: 'ws1' });
      for (let i = 0; i < 5; i++) {
        grantPermission({
          workspaceId: 'ws1',
      ...
      expect(getWorkspacePermissionsHistory('ws1', 3)).toHaveLength(3);
    });

    test('revokeAllWorkspacePermissions revokes all', () => {
      seedWorkspace({ id: 'ws1' });
      grantPermission({
        workspaceId: 'ws1',
        toolName: 'shell',
      ...
      const count = revokeAllWorkspacePermissions('ws1', null);
      expect(count).toBe(2);
      expect(getWorkspacePermissions('ws1')).toHaveLength(0);
    });
  });
