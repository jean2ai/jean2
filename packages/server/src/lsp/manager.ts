...
export class LSPManager {
  ...
  private diagnostics: DiagnosticsManager = new DiagnosticsManager();
  ...
  private async startClientIfNeeded(languageId: string): Promise<BaseLSPClient> {
    ...
    if (!this.workspaceRoot) {
      throw new Error('Workspace root not set. Call initialize() first.');
    }
  ...
  private handleDiagnosticsFromClient(uri: string, diagnostics: Diagnostic[]): void {
    this.diagnostics.updateDiagnostics(uri, diagnostics);
