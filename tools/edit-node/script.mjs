...
const { path: inputPath, oldString, newString, strategy, workspacePath, sessionId } = input;
...
function lineStartMatch(content, search) {
  const results = [];
  const lines = content.split('\n');
...
function lineEndMatch(content, search) {
  const results = [];
  const lines = content.split('\n');
...
function partialMatch(content, search) {
  const results = [];
...
function multiLineMatch(content, search) {
  const results = [];
...
async function editFile() {
  try {
    ...
    const strategies = ['exact', 'line_start', 'line_end', 'partial', 'multi_line'];
    let matches = [];
    let usedStrategy = '';
