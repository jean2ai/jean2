...
interface FileToMarkdownSecurityInput {
  args: {
    path: string;
  };
...
interface FileToMarkdownSecurityResult {
  allowed: boolean;
...
async function main() {
  try {
    ...
    const input: FileToMarkdownSecurityInput = JSON.parse(inputText);
    ...
    if (isBlockedPath(normalizedPath)) {
      const result: FileToMarkdownSecurityResult = {
        allowed: false,
